...
3. `int getCurrentEntity()`  
Возвращает идентификатор сущности, из которой работает скрипт
4. `void addScriptToEntity(string scriptName, int entityID)`  
Добавляет компонент script с именем scriptName на сущность с идентификатором entityID
5. `void destroyEntity(int entityID)`  
Уничтожает сущность с идентификатором EntityID
